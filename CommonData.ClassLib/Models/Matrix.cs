...
namespace CommonData.ClassLib.Models
{
    public class Matrix
    {
        ...
        /// <summary>
        /// Задает пустую, квадрутную матрицу размером N.
        /// </summary>
        public Matrix(int n)
        {
        ...
        /// <summary>Задает пустую, квадрутную матрицу размером N на М.</summary>
        ...
        public static Matrix Inverse(Matrix M, int round = 0)
        {
            ...
            for (int i = 0; i < M.Size; i++)
            {
                for (int j = 0; j < M.Size; j++)
                {
                    ...
                    matrix[j, i] = round == 0 ? (Math.Pow(-1, i + 1 + j + 1) / determinant) * Matrix.Determinant(tmp) : Math.Round(((1 / determinant) * Matrix.Determinant(tmp)), round, MidpointRounding.ToEven);
                }
            }
            return matrix;
        }
